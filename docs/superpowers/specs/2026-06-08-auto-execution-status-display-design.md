...
### popup（＝サイドパネル／サイドバー本体。chrome.json `side_panel`・firefox.json `sidebar_action` とも `popup/popup.html`）

**改訂（整理＋トグル化）**: 当初は `#status` に「最終スキャン / 自動 / BG / 続刊 / 値引き」を1行に連結していたが冗長。options と同じ折りたたみに統一する。

- `#status`（`<p>`）は**最終スキャン時刻のみ**＋操作中の一時メッセージ（既存の `setStatus` 用途）に戻す。自動/BG をここに連結しない。
- 自動実行の詳細は `#status` の下に**折りたたみ `<details>`**（`summary` = `statusHeading`、`open` 属性なし＝初期閉）で表示する。
  - 自動スキャン行: 最終＋次回（`autoScanStatusText`）
  - BGプローブ行: 最終＋次回＋続刊件数＋値引き件数（`bgProbeStatusText` ＋ 内訳）
- 構造（`<dl>` の行）と CSS は options の `.automation-status` を popup.css にミラーする。
- popup を開いた時点の値で表示（リアルタイム購読は不要）。i18n は既存キー（`statusHeading`/`autoScanStatusLabel`/`bgProbeStatusLabel`）を流用、新規文字列は不要。
...
内部用語が分かりづらいため、自動スキャン／BGプローブで語句を統一する。未実行の扱いも「(仮)」→「前回 なし／次回 …（予定）」に変更（enabledAt を「前回」に見せない）。

| キー | ja | en |
|------|----|----|
| `bgProbeStatusLabel` | 続刊・セール確認（旧: BGプローブ） | Next vol & sale |
| `statusEnabled` / `statusDisabled` | ON / OFF | ON / OFF |
| `statusLastRun(date)` | 前回 {date} | Last {date} |
| `statusLastNone`（新規） | 前回 なし | Last none |
| `statusNextRun(date)` | 次回 {date} | Next {date} |
| `statusProvisional` | （予定） | (est.) |
| `statusProgress(done,total)` | 確認中 {done}/{total} | Checking {done}/{total} |
| `statusBreakdown(next,disc)` | 続刊あり {next} / セール {disc} | Next vol {next} / Sale {disc} |

表示ロジック（options.js / popup.js 共通ヘルパ `lastNextParts`/`lastNextText`）:
- 実行済み（lastRun>0）→ 「前回 <日時>」「次回 <日時>」
- 未実行 → 「前回 なし」「次回 <enabledAt+間隔>（予定）」
