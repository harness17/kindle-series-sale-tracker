...
`kindle-series-sale-tracker` は **単一の Manifest V3 Chrome 拡張**（独立 git リポジトリ）。Amazon.co.jp の Kindle 蔵書一覧からシリーズ候補を抽出し、続刊確認用リストを作る。素の JavaScript で実装する。
...
**最新の引き継ぎ：** `CLAUDE_CODE_HANDOFF.md`（初回 handoff 発生時に作成）を参照する。
