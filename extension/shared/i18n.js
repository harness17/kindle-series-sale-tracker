(function (root, factory) {
  ...
  const TEXT = {
    ja: {
      ...
      bulkIndeterminateFailed: function (n) { return '判定不能が' + n + '件連続したため一括再確認を停止しました。既存の照会結果は保持しています。時間をおいて再実行してください。'; },
      bulkAborted: ' ／ 中止しました',
      checkAllBtn: '↻ 一括続刊再確認',
      ...
      csrfRejected: 'Amazon がリクエストを拒否しました（csrfToken 検証エラー）。この Kindle 一覧ページを再読み込み（F5）し、ログイン状態を確認してから再試行してください。',
    ...
    en: {
      ...
      sortCheckedAt: 'Has-next updated',
      ...
      bulkIndeterminateFailed: function (n) { return 'Stopped bulk recheck after ' + n + ' consecutive indeterminate results. Existing query results were kept. Please retry later.'; },
      bulkAborted: ' / Aborted.',
      ...
      sortPrice2: 'Next vol. price (lowest first)',
      sortCheckedAt2: 'Has-next updated (newest first)',
      sortTitle: 'Title (A–Z)',
      ...
      csrfRejected: 'Amazon rejected the request (csrfToken validation error). Please reload (F5) this Kindle library page, make sure you are signed in, and try again.',
