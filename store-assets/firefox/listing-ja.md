...
- Kindle Unlimitedとクーポンが併記された検索結果でも、購入時の実効価格を判定
- シリーズごとの優先表示、完結、除外フラグをローカル保存
- CSV / JSON でスキャン結果をエクスポート
- Firefoxサイドバーと専用ページで一覧を確認
- バックグラウンドで定期的に続刊・セール情報を自動チェック（オプトイン）
- 1回の定期実行で対象シリーズを全件巡回（Amazonへの負荷を抑えるため8件単位で順次処理）
- Kindle一覧ページ訪問時に蔵書を自動スキャン（オプトイン）
- 自動実行の状況（発火、実行中、完了、失敗、スキップ理由、続刊あり・セール件数）をサイドバーと専用ページで確認
...
新機能:
- 1回の定期実行で対象シリーズを全件確認。8件単位の負荷制御は維持
- 続刊・セール確認の実行中/完了/失敗、処理件数、失敗件数を表示
- 自動スキャンの発火、間隔未到来/基準データなし、進捗、完了、失敗を表示
- Kindle Unlimitedとクーポン併記時の実効価格計算を改善
- 新規パーミッションなし

過去の更新（0.4.0）:
- 自動スキャン／続刊・セール確認の実行状況表示を追加
- 起動/更新時のキャッチアップと簡易自動スキャンを追加
...
8. Open "専用ページ" and scroll to the "自動化" section. Enable "バックグラウンドで続刊・セールを確認" and "Kindle一覧ページ訪問時に自動スキャン".
